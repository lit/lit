...
suite('lit-html', () => {
  ...
  suite('html', () => {
    ...
    test.skip('does not create extra empty text nodes', () => {
      const countNodes =
    ...
    test('parses element-less text expression', () => {
      ...
      render(result, container);
      assert.equal(stripLitComments(container), '<div>1 2</div>');
    });
    ...
    test('parses expressions for two child nodes of one element', () => {
      ...
      render(result, container);
      assert.equal(stripLitComments(container), 'test');
    });
    ...
    test('parses expressions for two attributes of one element', () => {
      ...
      render(result, container);
      assert.equal(stripLitComments(container), '<div a="1" b="2"></div>');
    });
    ...
    test('updates when called multiple times with arrays', () => {
      ...
      assert.equal(
          stripLitComments(container), '<ul><li>a</li><li>b</li><li>c</li></ul>');
      render(ul(['x', 'y']), container);
      assert.equal(stripLitComments(container), '<ul><li>x</li><li>y</li></ul>');
    });
  ...
  suite('TemplateResult', () => {
    ...
    suite('first render', () => {
      ...
      test('renders a number', () => {
        ...
        stripLitComments(container);
        assert.equal(stripLitComments(container), '<div>123</div>');
      });
      ...
      test('renders nested templates', () => {
        ...
        stripLitComments(container);
        assert.equal(stripLitComments(container), '<h1>foo</h1>bar');
      });
      ...
      testSkipForTemplatePolyfill(
          'renders nested templates within table content', () => {
            ...
            assert.equal(
                stripLitComments(container), '<table><tr><td></td></tr></table>');

            table = html`<tbody>${html`<tr></tr>`}</tbody>`;
            render(table, container);
            assert.equal(stripLitComments(container), '<tbody><tr></tr></tbody>');
      ...
      testSkipSafari10_0(
          ...
          () => {
            ...
            assert.equal(stripLitComments(container), `<div a="<table>foo"></div>`);
          });

      test('values contain interpolated values', () => {
        const t = html`${'a'},${'b'},${'c'}`;
        render(t, container);
        assert.equal(stripLitComments(container), 'a,b,c');
      });

      // test('renders multiple nested templates', () => {
      //   const partial = html`<h1>${'foo'}</h1>`;
      //   html`${partial}${'bar'}${partial}${'baz'}qux`, container);
      //   assert.equal(stripComments(container),
      ...
      test('renders an element', () => {
        ...
        assert.equal(stripLitComments(container), '<div><p></p></div>');
      });
      ...
      test('renders an array of elements', () => {
        ...
        assert.equal(
            stripLitComments(container), '<div><p></p><a></a><span></span></div>');
      });
      ...
      test(
          ...
          () => {
            ...
            assert.equal(stripLitComments(container), '<div a="b" foo="bar"></div>');
          });
      ...
      test('renders to multiple attribute expressions', () => {
        render(
            ...
            container);
        assert.oneOf(stripLitComments(container), [
      ...
      test('renders to attributes with attribute-like values', () => {
        ...
        assert.equal(stripLitComments(container), '<div foo="bar=foo"></div>');
      });
      ...
      test('renders racing Promises correctly', () => {
        ...
        return promise1.then(() => {
          ...
          return promise2.then(() => {
            assert.equal(stripLitComments(container), '<div>bar</div>');
          });
        });
      });
      ...
      test('renders expressions with preceding elements', () => {
        ...
        assert.equal(stripLitComments(container), '<a>foo</a><h1>bar</h1>');

        // This is nearly the same test case as above, but was causing a
        // different stack trace
        render(html`<a>${'foo'}</a>${'bar'}`, container);
        assert.equal(stripLitComments(container), '<a>foo</a>bar');
      });
      ...
      test('renders directives on AttributeParts', () => {
        ...
        assert.equal(stripLitComments(container), '<div foo="foo"></div>');
      });
    ...
    suite('update', () => {
      ...
      test('sanity check one', () => {
        ...
        render(t(), container);
        assert.equal(stripLitComments(container), '<div>aaa</div>');
        const text = container.firstChild!.childNodes[1] as Text;
        text.textContent = 'bbb';
        render(t(), container);
        assert.equal(stripLitComments(container), '<div>bbb</div>');
      });
      ...
      test('renders to and updates sibling parts', () => {
        ...
        foo = 'bbb';
        render(t(), container);
        assert.equal(stripLitComments(container), '<div>bbbbar</div>');
      });
      ...
      test('renders and updates attributes', () => {
        ...
        render(t(), container);
        assert.equal(stripLitComments(container), '<div a="foo:bar"></div>');

        foo = 'bbb';
        render(t(), container);
        assert.equal(stripLitComments(container), '<div a="bbb:bar"></div>');
      });
      ...
      test('updates nested templates', () => {
        ...
        assert.equal(stripLitComments(container), '<h1>foo</h1>baz');

        foo = 'bbb';
        render(t(true), container);
        assert.equal(stripLitComments(container), '<h1>bbb</h1>baz');

        render(t(false), container);
        assert.equal(stripLitComments(container), '<h2>bar</h2>baz');
      });
      ...
      test('updates arrays', () => {
        ...
        items = [3, 2, 1];
        render(t(), container);
        assert.equal(stripLitComments(container), '<div>321</div>');
      });
      ...
      test('updates arrays that shrink then grow', () => {
        ...
        items = [4];
        render(t(), container);
        assert.equal(stripLitComments(container), '<div>4</div>');

        items = [5, 6, 7];
        render(t(), container);
        assert.equal(stripLitComments(container), '<div>567</div>');
      });
      ...
      test('updates an element', () => {
        ...
        render(t(), container);
        assert.equal(stripLitComments(container), '<div><p></p><div></div></div>');

        child = undefined;
        render(t(), container);
        assert.equal(stripLitComments(container), '<div><div></div></div>');

        child = document.createTextNode('foo');
        render(t(), container);
        assert.equal(stripLitComments(container), '<div>foo<div></div></div>');
      });
  ...
  suite('NodePart', () => {
    ...
    suite('setValue', () => {
      ...
      test('accepts a string', () => {
        part.setValue('foo');
        assert.equal(stripLitComments(container), 'foo');
      });

      test('accepts a number', () => {
        part.setValue(123);
        assert.equal(stripLitComments(container), '123');
      });
      ...
      test('accepts an element', () => {
        ...
        assert.equal(stripLitComments(container), '<p></p>');
      });
      ...
      test('accepts nested templates', () => {
        ...
        assert.equal(stripLitComments(container), '<h1>foo</h1>');
      });
      ...
      test('accepts an array of elements', () => {
        ...
        assert.equal(stripLitComments(container), '<p></p><a></a><span></span>');
      });
      ...
      test('updates a simple value to a complex one', () => {
        ...
        render(t(), container);
        assert.equal(stripLitComments(container), '<div><span>bar</span></div>');
      });
      ...
      test('updates when called multiple times with simple values', () => {
        part.setValue('abc');
        assert.equal(stripLitComments(container), 'abc');
        part.setValue('def');
        assert.equal(stripLitComments(container), 'def');
      });
      ...
      test('updates when called multiple times with arrays 2', () => {
        ...
        part.setValue([4, 5]);
        assert.equal(stripLitComments(container), '45');
        assert.deepEqual(
            ['', '4', '', '5', ''],
            Array.from(container.childNodes).map((n) => n.nodeValue));
        assert.strictEqual(container.firstChild, startNode);
      ...
      test('updates arrays with siblings', () => {
        ...
        render(t(), container);
        assert.equal(stripLitComments(container), '<p></p>123<a></a>');

        items = [1, 2, 3, 4];
        render(t(), container);
        assert.equal(stripLitComments(container), '<p></p>1234<a></a>');
      });
      ...
      test(
          ...
          () => {
            ...
            value = 'bar';
            part.setValue(r());
            assert.equal(stripLitComments(container), '<h1>bar</h1>');
      ...
      test(
          ...
          () => {
            ...
            part.setValue(r());
            assert.equal(stripLitComments(container), '<li>1</li><li>2</li><li>3</li>');
            const originalLIs = Array.from(container.querySelectorAll('li'));

            items = [3, 2, 1];
            part.setValue(r());
            assert.equal(stripLitComments(container), '<li>3</li><li>2</li><li>1</li>');
