...
export const renderTests = (render: Render) => {
  suite('render()', () => {
    ...
    suite('text', () => {
      ...
      // TODO(justinfagnani): add this test back with a test TemplateProcessor
      // and html tag.
      test.skip(
    ...
    suite('events', () => {
      ...
      test('allows updating event listener', () => {
        ...
        const t = (listener: () => void) =>
            html`<div @click=${listener}></div>`;
    ...
    suite('directives', () => {
      ...
      testSkipChrome41(test)(
          'event listeners can see events fired by dynamic children', () => {
            // This tests that node directives are called in the commit phase,
            // not the setValue phase
            let event: Event|undefined = undefined;
            document.body.appendChild(container);
            render(
                html`
    ...
    suiteIfCustomElementsAreSupported('custom elements', () => {
      ...
      test(
          ...
          () => {
            // Container must be in the document for the custom element to
            // upgrade
            document.body.appendChild(container);
            ...
            const instance =
                container.firstElementChild as PropertySetterElement;
            assert.equal(instance.value, 'foo');
            assert.isTrue(instance.calledSetter);
          });
      ...
      test(
          ...
          () => {
            // Here we _don't_ connect the container to the document, to
            // highlight that we aren't creating upgraded custom elements in
            // this case when we ideally should be.
            const template = (content: any) => html`${content}`;
    ...
    suite('updates', () => {
      ...
      test('updates arrays that shrink then grow', () => {
        ...
        render(t(), container);
        assert.equal(
            stripExpressionMarkers(container.innerHTML), '<div>4</div>');
