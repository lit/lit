...
export class NodePart implements Part {
  ...
  appendIntoPart(part: NodePart) {
    part.__insert(this.startNode = createStartMarker());
    part.__insert(this.endNode = createEndMarker());
  }
  ...
  insertAfterPart(ref: NodePart) {
    ref.__insert(this.startNode = createStartMarker());
  ...
  private __commitTemplateResult(value: TemplateResult): void {
    ...
    if (this.value instanceof TemplateInstance &&
      ...
      // TemplateResult's template during SSR.
      const markerWithDigest = `lit-part ${value.digest}`;
      if (this.options.hydrate && this.startNode.textContent === markerWithDigest) {
        ...
        // In "SSR" mode we always render two markers and write the template
        // digest to the start marker so that we can compare during hydration.
        // This is only for testing, and only for now, since real SSR is done
        // with a different server implementation.
        if (this.options.ssr) {
          this.startNode.textContent = markerWithDigest;
        }
      }
      this.value = instance;
    }
