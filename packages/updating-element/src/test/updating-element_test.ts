...
suite('UpdatingElement', () => {
  ...
  if (DEV_MODE) {
    suite('Developer mode warnings', () => {
      ...
      const defaultWarningCount = missingPlatformSupport ? 1 : 0;
      ...
      test('warning for platform-support', async () => {
        class A extends UpdatingElement {}
        customElements.define(generateElementName(), A);
        const a = new A();
        container.appendChild(a);
        await a.updateComplete;
        assert.equal(warnings.length, defaultWarningCount);
      ...
      test('warns when `static render` is implemented', () => {
        ...
        customElements.define(generateElementName(), A);
        new A();
        assert.equal(warnings.length, defaultWarningCount + 1);
