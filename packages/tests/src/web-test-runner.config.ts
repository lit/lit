...
export class SaucelessLauncher extends WebdriverLauncher {
  constructor(name: string, options: RemoteOptions) {
    super(options);
    this.name = name;
  }
  ...
  startSession(sessionId: string, url: string) {
    return super.startSession(
      sessionId,
      url.replace(/(localhost|127\.0\.0\.1)/, networkAddress)
    );
  }
...
export function createSauceLabsLauncher(
  ...
  if (typeof finalConnectOptions.tunnelIdentifier !== 'string') {
    throw new Error('Please provide a tunnelIdentifier');
  ...
  return function sauceLabsLauncher(
    ...
    const browserName =
      caps.browserName ?? caps.browser ?? caps.device ?? 'unknown';
    const browserVersion =
      caps.browserVersion ?? caps.version ?? caps.browser_version ?? '';
    ...
    return new SaucelessLauncher(browserIdentifier, options);
  };
}
