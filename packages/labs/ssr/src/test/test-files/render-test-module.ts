...
export const trivialServerOnly = html`<div>Server only</div>`;

export const serverOnlyWithBinding = html`<div>${'Server only'}</div>`;

export const serverOnlyInsideServerOnly = html`<div>${html`Server only`}</div>`;

export const serverOnlyRawElementTemplate = html`
  <title>${'No'} comments ${'inside'}</title>
  <textarea>${'This also'} works${'.'}</textarea>
`;

export const serverOnlyInTemplateElement = html`
  <template
    >${'one'}
    <div>
      ${'two'}
      <div>${'three'}</div>
      <template>${'recursed'}</template>
    </div></template
  >
`;

export const serverOnlyDocumentTemplate = html`
  <!DOCTYPE html>
  <html>
    <head>
      <title>${'No'} comments ${'inside'}</title>
...
export const serverOnlyArray = html`<div>${['one', 'two', 'three']}</div>`;

export const serverOnlyRenderHydratable = html`
  <div>${'server only'}</div>
...
export const hydratableRenderServerOnly = html`
  <div>${'dynamic!'}</div>
  ${html`<div>${'one time'}</div>`}
`;

export const serverOnlyRenderPropertyBinding = html`<div
  .foo=${'server only'}
></div>`;

export const serverOnlyRenderEventBinding = html`<div
...
export const renderServerOnlyScript = html` <script>
  console.log('${'This is dangerous!'}');
</script>`;

export const renderServerOnlyScriptDeep = html` <script>
  <div>console.log("${'This is dangerous!'}");</div>;
</script>`;

export const renderServerOnlyStyle = html` <style>
  div {
    color: ${'red'};
  }
</style>`;

export const renderServerOnlyStyleDeep = html` <style>
  <div>
    color: ${'red'};
  </div>
</style>`;

export const renderServerScriptNotJavaScript = html` <script type="json">
  {"ok": ${true}}
</script>`;

// This doesn't have to make sense, the test is that it'll throw at the
// template preparation phase.
export const renderServerOnlyElementPart = html`<div ${'foo'}></div>`;
