...
export class Virtualizer {
  ...
  private _updateView() {
    ...
    if (hostElement && hostElement.isConnected && scrollingElement && layout) {
      ...
      // equivalent) and should render nothing. All other elements — including
      // newly-mounted ones not yet laid out — use a floor of 1 to bootstrap
      // the rendering cycle.
      const isHidden =
  ...
  private _sizeHostElement(size: VirtualizerSize | null) {
    ...
    //   Scroller: the host has fixed external sizing, so the cross axis
    //     is only used for the sizer element. 'min' → "Npx" (content
    //     needs at least this much space), 'max' → "0px" (no constraint).
    //   Non-scroller: the host self-sizes, so we emit a CSS function:
    //     "min(100%, Npx)" or "max(100%, Npx)" to balance content size
