...
export class FlowLayout extends BaseLayout<BaseLayoutConfig> {
  ...
  _getInlineSize(idx: number): number {
    const item = this._getPhysicalItem(idx);
    return item ? this._metricsCache.getInlineSize(idx) : 0;
  }
  ...
  _getItems() {
    ...
    let inlineSize = this._getInlineSize(this._anchorIdx);
    ...
    while (this._physicalMin > lower && this._first > 0) {
      ...
      inlineSize = Math.max(inlineSize, this._getInlineSize(this._first));
    ...
    while (this._physicalMax < upper && this._last < this.items.length - 1) {
      ...
      inlineSize = Math.max(inlineSize, this._getInlineSize(this._last));
