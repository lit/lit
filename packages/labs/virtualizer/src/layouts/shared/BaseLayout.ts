...
export abstract class BaseLayout<C extends BaseLayoutConfig> implements Layout {
  ...
  /**
   * The maximum inline size across all currently rendered items, in pixels.
   * Used to size the host element on the non-scrolling axis in non-scroller
   * mode.
   */
  protected _inlineSize: number | null = 100;
  ...
  protected _sendStateChangedMessage() {
    ...
    // In non-scroller mode the Virtualizer self-sizes its host element on
    // the cross axis. The strategy depends on writing mode:
    //
    // horizontal-tb → cross axis is width → max(100%, Npx)
    //   Fill the containing block, or grow wider if items are wider.
    //
    // vertical-lr/rl → cross axis is height → min(100%, Npx)
    //   Shrink-wrap to content height, but don't exceed the container.
    //   Without the min(), a vertical-mode virtualizer would force the
    //   host taller than the viewport.
    ...
    const message: StateChangedMessage = {
      ...
      virtualizerSize: {
        ...
        inlineSize: [minOrMax, clampSize(this._inlineSize!)],
      } as VirtualizerSize,
      range: {
        first: this._first,
        last: this._last,
