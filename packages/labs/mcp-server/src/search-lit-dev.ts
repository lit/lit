...
export function registerSearchLitDevDocsTool(server: McpServer) {
  server.tool(
    'search_lit_dev_docs',
      ...
      'to your agent to fetch the content of the URL to enhance the results.',
    {
      ...
      maxResults: z
        ...
        .optional()
        .default(5)
        .describe('Maximum number of distinct results to return (default: 5).'),
      page: z
        .number()
        .int()
        .min(0)
        .optional()
        .default(0)
        .describe('Page number of results to return (0-indexed, default: 0).'),
    },
    async ({
      query,
      maxResults,
      ...
      try {
        ...
        return {
          content: [{type: 'text', text: JSON.stringify(cleanedHits, null, 2)}],
        };
      } catch (error) {
