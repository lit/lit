...
const testProject = (project: string, output: string) => {
  test('basic wrapper generation', async () => {
    ...
    const wrapperSourceFile = fs.readFileSync(
      path.join(outputPackage, `src/${output}.ts`)
    );
...
testProject('test-element-a', 'element-a');
testProject('test-element-events', 'element-events');
testProject('test-element-props', 'element-props');
testProject('test-element-slots', 'element-slots');
