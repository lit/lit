...
export class LitCli {
  ...
  constructor(args: string[], options: Options) {
    ...
    // If the "--skip-permissions" flag is ever present, it will skip
    // asking for permission to install commands that are not installed.
    if (args.indexOf('--skip-permissions') > -1) {
