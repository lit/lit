...
suite('HTML Parser Special Attributes', () => {
  ...
  suite('Mixed Attribute Types', () => {
    test('parses template with mixed attribute types', () => {
      // Create expressions for the test
      harness.testParse(
        `<my-element id="regular-attr"></my-element>`,
        (fragment) => {
          harness.assertChildCount(fragment, 1);
          const element = fragment.childNodes[0] as Element;
    ...
    // Test each special attribute type separately since the combined test is more complex
