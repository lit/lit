...
function serializeOpenCloseTags(node: ChildNode): {
  ...
  const withoutChildren: ChildNode = {...node, childNodes: []};
  const fakeParent = {childNodes: [withoutChildren]} as unknown as ParentNode;
...
function serializeComment(comment: CommentNode): string {
  return parse5.serialize({childNodes: [comment]} as unknown as ParentNode);
}
