...
import {upgrade} from 'lit/directives/upgrade.js';
/**
 * @license
  ...
  render() {
    return html`
      <div class="container style-scope x-thing">
        <span class="from style-scope x-thing">${this.from}</span>
        <span class="time style-scope x-thing">${this.time}</span>
        <div class="subject style-scope x-thing">${this.subject}</div>
      </div>
    `;
  }
};
...
let XItem = class XItem extends LitElement {
  render() {
    return html`
      <div @click="${this.onClick}" class="item style-scope x-item">
        <x-thing
          .from="${this.item.value0}"
          .time="${this.item.value1}"
          .subject="${this.item.value2}"
          class="style-scope x-item"
          ${upgrade()}
        ></x-thing>
        <x-thing
          undefined="${this.item.value3}"
          .from="${this.item.value4}"
          .time="${this.item.value5}"
          class="style-scope x-item"
          ${upgrade()}
        ></x-thing>
        <x-thing
          .subject="${this.item.value6}"
          undefined="${this.item.value7}"
          .from="${this.item.value8}"
          class="style-scope x-item"
          ${upgrade()}
        ></x-thing>
        <x-thing
          .time="${this.item.value9}"
          .subject="${this.item.value10}"
          undefined="${this.item.value11}"
          class="style-scope x-item"
          ${upgrade()}
        ></x-thing>
        <x-thing
        ...
        ></x-thing>
        <x-thing
          undefined="${this.item.value15}"
          .from="${this.item.value16}"
          .time="${this.item.value17}"
          class="style-scope x-item"
          ${upgrade()}
        ></x-thing>
      </div>
...
(async () => {
  ...
  let el;
  const create = () => {
    render(html`<x-app ${upgrade()}></x-app>`, document.body);
