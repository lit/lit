...
suite('UpdatingElement', () => {
  ...
  suite('lifecycle callbacks', () => {
    ...
    test('update/updated callbacks', async () => {
      let updateCount = 0;
      let updateChangedPropeties: PropertyValues | null = null;
      let updatedChangedPropeties: PropertyValues | null = null;
      el.updateCallbacks.add((changedProperties: PropertyValues) => {
        updateChangedPropeties = changedProperties;
        updateCount++;
      });
      let updatedCount = 0;
      el.updatedCallbacks.add((changedProperties: PropertyValues) => {
        updatedChangedPropeties = changedProperties;
        updatedCount++;
      });
      assert.equal(el.updateCount, 1);
      ...
      assert.deepEqual(updateChangedPropeties, expectedChangedProperties);
      assert.equal(updatedCount, 1);
      assert.deepEqual(updatedChangedPropeties, expectedChangedProperties);
    });

    test('callbacks can only be added once', async () => {
      let connectedCount = 0;
