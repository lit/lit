...
export interface TemplateRenderer {
  ...
  openTagStart: (s: string, level: string, tag: string, attrCount: number, staticAttrCount: number) => string;
  setAttr: (s: string, level: string, name: string, isStatic: boolean, isConstant: boolean, valPerDynAttr: number) => string;
  openTagEnd: (s: string, level: string, tag: string, attrCount: number) => string;
  textNode: (s: string, level: string, isStatic: boolean, value: string) => string;
  closeTag: (s: string, level: string, tag: string) => string;
  callTemplate: (s: string, level: string, name: string, isConstant: boolean) => string;
  endTemplate: (s: string, level?: string) => string;
  render: () => string;
  endBenchmark: () => string;
}

export const getRenderers = (opts: {pretty: boolean}): {[key: string]: TemplateRenderer} => {
  
  // Returns padding for pretty-printing appropriate for the depth of the level,
  // or nothing if pretty printing is disabled
  const indent = (level: string, more: number = 0) => {
  ...
  `;
    ...
    openTagStart: (s: string, level: string, tag: string, _attrCount: number, _staticAttrCount: number) => {
      return s + `${indent(level)}<${tag}`;
    },
    setAttr(s: string, _level: string, name: string, isStatic: boolean, isConstant: boolean, valPerDynAttr: number) {
      if (isStatic) {
        return s + ` a${name}="${name}"`;
      } else {
        return s + ` a${name}="` + Array(valPerDynAttr).fill(0).map((_, i) => {
    ...
    callTemplate(s: string, level: string, name: string, isConstant: boolean) {
      if (isConstant) {
        ...
        return s + `${indent(level, 1)}\${useTemplateA ? ${name}A() :  ${name}B()}${cr()}`;
      }
    },
    endTemplate(s: string, _level: string = '') {
      return s + `\`;\n`;
    },
    render() {
  ...
  `;
    ...
    openTagStart(s: string, level: string, tag: string, attrCount: number, _staticAttrCount: number) {
      if (attrCount > 0) {
    ...
    setAttr(s: string, level: string, name: string, isStatic: boolean, isConstant: boolean, valPerDynAttr: number) {
      if (isStatic) {
        return s + `${indent(level)}attr('a${name}', '${name}');${cr()}`;
      } else {
        return s + `${indent(level)}attr('a${name}', ` + Array(valPerDynAttr).fill(0).map((_, i) => {
          const mod = valPerDynAttr > 1 ? `:${i}` : '';
          return `'${name}${mod}${isConstant ? `'` : `#' + updateId`}`;
        }).join(` + '-' + `) + ');' + cr();
      }
    ...
    callTemplate(s: string, level: string, name: string, isConstant: boolean) {
      if (isConstant) {
        ...
        return s + `${indent(level, 1)}if (useTemplateA) { ${name}A(); } else { ${name}B(); };${cr()}`;
      }
    },
    endTemplate(s: string, _level: string = '') {
      return s + `};\n`;
  ...
  const idomKeyedStaticsRenderer: TemplateRenderer = {
    ...idomRenderer,
    openTagStart(s: string, level: string, tag: string, attrCount: number, staticAttrCount: number) {
      if (staticAttrCount > 0) {
        const statics = Array(staticAttrCount).fill(0).map((_, attrIndex) => {
          const attrLevel = nextLevel(level, attrIndex);
          return `'a${attrLevel}', '${attrLevel}'`;
        }).join(',');
        if (attrCount > staticAttrCount) {
          return s + `${indent(level)}elementOpenStart('${tag}', '${level}', [${statics}]);${cr()}`;
        } else {
          return s + `${indent(level)}elementOpen('${tag}', '${level}', [${statics}]);${cr()}`;
        }
      } else {
        return idomRenderer.openTagStart(s, level, tag, attrCount, staticAttrCount);
      }
    },
    setAttr(s: string, level: string, name: string, isStatic: boolean, isConstant: boolean, valPerDynAttr: number) {
      if (isStatic) {
        ...
        return idomRenderer.setAttr(s, level, name, isStatic, isConstant, valPerDynAttr);
      }
    },
  };
  ...
  `;
    ...
    openTagStart: (s: string, level: string, tag: string, attrCount: number, _staticAttrCount: number) => {
      return s + `${indent(level)}createElement('${tag}', ${attrCount ? '{' : 'null'}`;
    },
    setAttr(s: string, _level: string, name: string, isStatic: boolean, isConstant: boolean, valPerDynAttr: number) {
      if (isStatic) {
        return s + ` a${name}: '${name}',`;
