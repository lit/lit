...
@customElement('foo-virtualizer')
export class FooVirtualizer<Item, Child extends HTMLElement> extends LitElement {
    @property()
    ...
    render() {
        console.log(this._renderItem, this._first, this._last, this._items, this._scroller)
        return html`Foo`;
    }
}
...
export class LitVirtualizer<Item, Child extends HTMLElement> extends LitElement {
    ...
    render(): TemplateResult {
        ...
        const itemsToRender = items.slice(_first, _last + 1);
        return html`
