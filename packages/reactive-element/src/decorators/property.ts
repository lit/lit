 ...
 * property when clicked and fires a `changed` event. Mutating public properties
 * should typically not be done for non-primitive (object or array) properties.
 * In other cases when an element needs to manage state, a private property
 * decorated via the [[`state`]] decorator should be used. When needed, state
 * properties can be initialized via public properties to facilitate complex
 * interactions.
 *
 * @example
 * ```ts
