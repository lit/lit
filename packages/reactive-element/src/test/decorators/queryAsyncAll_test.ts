...
(canTestReactiveElement ? suite : suite.skip)('@queryAsyncAll', () => {
  ...
  class C extends RenderingElement {
    @queryAsyncAll('div') divs!: Promise<NodeListOf<HTMLDivElement>>;

    @queryAsyncAll('span') spans!: Promise<NodeListOf<HTMLSpanElement>>;

    @queryAsyncAll('.foo') foos!: Promise<NodeListOf<Element>>;
