...
In some cases you will want `x` to overwrite what the user has typed in. lit-html tracks the previous value of a binding and only updates the DOM is the value has changed. Without the live directive, lit-html might not think that the `value` property of the input has changed and won't update the DOM.
...
We still reccomend that you handle user input so that the state and the DOM agree:
